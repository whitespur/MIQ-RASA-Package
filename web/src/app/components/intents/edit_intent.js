...
function EditIntentController($rootScope, $scope, Agent, AgentEntities, Intent, Expressions, Expression, Parameter, Parameters, Entities, UniqueIntentEntities,Responses, Response, IntentTags, Bind, $sce ) {
...
function startBlockView(el, data) {
  ...
  $.each(blocks, function(i,v) {
    ...
    parent.find('.text-block-container').append("<div class='single-block'><div class='innerTaskbar'><a onclick='$(this).parent().parent().remove()'>X</a></div><div class='textblock' contentEditable='true' ng-mouseup='showTextTaskbar($event)'>" + escapeHtml(v) + "</div></div>");
  })
}
...
$scope.saveIntentResponseBlocks = function(e) {
  ...
  blocks.each(function(i,v) {
    if($(this).find('textarea')[0].html !== $scope.default_textarea_text) {
      html += unescapeHtml($(this).find('textarea')[0].html) + '</block>';
    }
  })
