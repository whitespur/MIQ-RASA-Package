...
function EditIntentController($rootScope, $scope, Agent, AgentEntities, Intent, Expressions, Expression, Parameter, Parameters, Entities, UniqueIntentEntities,Responses, Response, IntentTags, Bind, $sce ) {
  ...
  $scope.editExpression = function(expression_id) {
    ...
    span.parent().find('> span:last-child').prepend('<button class="btn btn btn-outline-danger" ng-click="doUpdateExpression(expression.expression_id)"><span class="icon-check"></span></button>');
