...
function InsightsController($scope, $http, $sce, CurrentAccount, NLU_log_stats) {
  ...
  NLU_log_stats.query({path: "avgNluResponseTimesLast30Days"}, function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      nlu_avg_response = nlu_avg_response + parseInt(data[i].round);
    }
  ...
  NLU_log_stats.query({path: "avgUserResponseTimesLast30Days"}, function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      user_avg_response = user_avg_response + parseInt(data[i].round);
    }
    $scope.user_avg = parseInt(user_avg_response / data.length);
  ...
  NLU_log_stats.query({path: "intentsMostUsed"}, function (data) {
    ...
    if (agents_dropdown.length > 0) {
      $scope.intents_pie_labels = agents_intents_map.get(agents_dropdown[0]).label_data;
  ...
  $scope.updatePieChartData = function (agent_name) {
    $scope.intents_pie_labels = $scope.agents_intents_map.get(agent_name).label_data;
  ...
  function loadDailyActiveUsers() {
    NLU_log_stats.query({path: "activeUserCountLast30Days"}, function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        avg_act_users = avg_act_users + parseInt(data[i].user_count);
      }
      $scope.avg_active_users = parseInt(avg_act_users / data.length);
  ...
  function loadMonthlyActiveUsers() {
    NLU_log_stats.query({path: "activeUserCountLast12Months"}, function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        avg_act_users = avg_act_users + parseInt(data[i].count_users);
      }
      $scope.activeusers_data = [users_data];
      $scope.avg_active_users = parseInt(avg_act_users / data.length);
    });
