...
function parseRequest(req, res, next, agentObj) {
  ...
  if(req.body.model != undefined){
    ...
    db.any('SELECT agent_id, model_name FROM agents WHERE agent_name = ' + req.body.project + ' INNER JOIN agent_models ON agent_models.agent_id = agents.agent_id ORDER BY created_at asc LIMIT 1')
...
// ----------------------------------------------------------
function finalizeCacheFlushToDbAndRespond(cacheKey, http_code, res, body, req) {
        ...
        if (nlu_parse_cache.agent_id != undefined) {
          db.any('insert into messages(agent_id, user_id, user_name, message_text, message_rich, user_message_ind)' +
            ...
            .then(function (returnData) {
              ...
              db.none('INSERT INTO nlu_parse_log(intent_name, entity_data, messages_id,intent_confidence_pct, user_response_time_ms,nlu_response_time_ms) '+
...
function updateAndSendRasaResponse(req, cacheKey, rasa_response, modelName, projectName, res) {
  if (rasa_response.intent == undefined) {
    finalizeCacheFlushToDbAndRespond(cacheKey,200, res, rasa_response, req);
  } else {
    db.any(
