...
var pages = {
  ...
  requestUserPermission = function(username, page_name,next, res, req) {
    ...
    db.one("SELECT account_type_id, account_type_permissions.name as name, account_type_permissions.level FROM account JOIN account_type_permissions ON account_type_id::int = user_id WHERE name = '" + page_name + "' AND username = '" + username + "'")
