...
var pages = {
  ...
  requestUserPermission = function(username, page_name,next, res) {
    ...
    console.log("SELECT account_type_id, account_type_permissions.name as name, level FROM account JOIN account_type_permissions ON account_type_id::int = user_id WHERE name = '" + url + "' AND username = '" + username + "'");
